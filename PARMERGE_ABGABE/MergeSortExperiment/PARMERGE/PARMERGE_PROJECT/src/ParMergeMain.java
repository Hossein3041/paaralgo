...
public class ParMergeMain {
    ...
    private static class ParMergeSort extends RecursiveAction {
        ...
        protected void compute() {
            int length = r - p + 1;
			if (length <= SEQ_TRESHOLD) {
                Arrays.sort(a, p, r + 1); // Hier Array.sort();
			} else {
                //final int q = (p + r) / 2;
				//final int q = p + (r - p) / 2;
                int q = (p >>> 1) + (r >>> 1) + ((p & 1) & (r & 1));
    ...
    public static class ParMergeAux extends RecursiveAction {
        ...
        protected void compute() {
            if (p1 > r1)
                ...
                if (total <= MERGE_TRESHOLD)
					...
					//final int q1 = p1 + (r1 - p1) / 2;
                    int q1 = (p1 >>> 1) + (r1 >>> 1) + ((p1 & 1) & (r1 & 1));
					final int x = a[q1];
        ...
        protected int findSplitPoint(int[] a, int p, int r, int x) {
            ...
            while (low < high) {
                //int mid = (low + high) / 2;
                //int mid = (low >>> 1) + (high >>> 1) + ((low & 1) & (high & 1)); // >>> wäre für negative Zahlen falsch
				int mid = (low >>> 1) + (high >>> 1); // Auch ok?
				if (x <= a[mid]) {
                    high = mid;
                } else {
                    low = mid + 1;
