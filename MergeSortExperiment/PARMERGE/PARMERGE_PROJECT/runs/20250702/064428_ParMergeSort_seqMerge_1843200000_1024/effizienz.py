...
def plot_bereich(df, x_min, x_max, filename_suffix, title_suffix, N_str, Pmax_str, modus, name, use_log_x):
    ...
    plt.plot(df_filtered['P'], df_filtered['Efficiency'], marker='o',
    ...
    plt.title(f"Effizienz vs Threads {title_suffix}")
    plt.ylim(0, 1.05)
...
def main(tsv_path):
    ...
    name = name.replace("laufzeit", "effizienz")  # Output-Name anpassen
